'use strict';
var app = angular.module('app', ['ngRoute', 'ngResource'])//app 모듈 선언

.constant('config', //미국 각 주의 약어 목록
...
app.config(['$routeProvider', function($routeProvider){
   $routeProvider.when('/', { templateUrl : 'home.html' })
                        .when('/employees',  // employees 요청
                              { templateUrl : 'employees.html', controller : 'EmployeesCtrl' })
                        .when('/employees/:employeeId',  // employees/employeeId 요청
                              { templateUrl : 'employee.html', controller : 'EmployeeCtrl' })
                        .when('/teams',  // teams 요청
                              { templateUrl : 'teams.html', controller : 'TeamsCtrl' })
                        .when('/teams/:teamId',  // teams/teamId 요청
                              { templateUrl : 'team.html', controller : 'TeamCtrl' })
                        .otherwise({ redirectTo : '/' });
}]);

//EmployeeService - http 호출 전체 대체
...
app.controller('EmployeeCtrl', ['$scope', '$routeParams', 'EmployeeService', 'TeamService', '$q', 'config', '$route',
   function($scope, $routeParams, employee, team, $q, config, $route){
      ...
      $q.all([employee.get({ employeeId : $routeParams.employeeId}).$promise,
         team.query().$promise]).then(function(values){
